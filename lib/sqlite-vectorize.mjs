...
export class VectorDB {
	...
	async find(txt, filter = {}, limit = 10) {
		...
		let results = this.getSemanticSearchStmt.all(queryEmbeddingBuffer, effectiveLimit);

		if (Object.keys(filter).length > 0) {
			results = results.filter((row) => {
				try {
					const rowMetadata = JSON.parse(row.metadata);
					for (const key in filter) {
						if (Object.prototype.hasOwnProperty.call(filter, key) && rowMetadata[key] !== filter[key]) {
							return false;
						}
					}
					return true;
				} catch (e) {
					this.logger.warn(`Could not parse metadata for row ${row.iobroker_text_id}: ${e.message}`);
					return false;
				}
			});
		}
