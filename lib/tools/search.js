...
export const search = {
    ...
    desc: DESC,
    params: {
        text: z.string().describe("natural language query to match against the names, descriptions, roles, rooms, functions, and other meta information of state objects."),
        filter: z.record(z.any()).optional().default({}).describe("An optional filter object to narrow down the search results."),
        limit: z.number().optional().default(10).describe("The maximum number of results to return. it has meaningful defaults of 10. ommit or set higher if you don't find what you are looking for")
    },
    call: API => async (args) => {
        let data = await API.search(args.text,args.filter || {},args.limit || 10)
        return {
            content: [
                {
                    type: "text",
