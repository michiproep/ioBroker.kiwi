...
export const getStatesWithValues = {
    ...
    desc: DESC,
    params: {
        instancePattern: z.string().describe("The pattern to match the instance name. Default is '*' which matches all instances. the pattern can have a wildcard charcter ("*") anywhere but at the start of the pattern"),
    },
    call: API => async (args) => {
        let ret = await API.getStatesWithValues(args)
        return {
            content: [
                {
                    type: "text",
