...
export const getHistory = {
    ...
    desc: DESC,
    params: {
        id: z.string().describe("The ID of the state to get history data for. This is usually in the format {instance}.{adapter}.{state} (e.g., 'hue.0.brightness')."),
        options: z.object({
            ...
            aggregate: z.enum(["minmax", "none", "average", "total", "min", "max", "count", "percentile", "quantile", "integral"]).describe("Aggregation method.").optional(),
            ...
            integralInterpolation: z.enum(["linear", "step"]).describe("Interpolation method for integral aggregation.").optional(),
        }).describe("Options for history query."),
    },
    call: API => async (args) => {
        let res = await API.getHistory(args)
        return {
            content: [
                {
                    type: "text",
