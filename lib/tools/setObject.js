...
export const setObject = {
    ...
    desc: DESC,
    params: {
        id: z.string().describe("The ID of the object to get. This is usually in the format {instance}.{adapter}.{name} (e.g., 'hue.0.brightness')."),
        obj: z.record(z.any()).describe("The object to set for the state.example: {type: 'state', common: {name: 'Brightness', type: 'number', role: 'level.brightness'}, native: {}, acl: {read: true, write: true}}"),
    },
    call: API => async (args) => {
        let ret = await API.setObject(args)
        return {
            content: [
                {
                    type: "text",
