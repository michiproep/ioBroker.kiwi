...
export const createState = {
    ...
    desc: DESC,
    params: {
        id: z.string().optional().describe("The ID of the object to create. This can be a device, channel, state or folder id."),
        name: z.string().describe("The display name of the state."),
        type: z.string().optional().default("string").describe("The type of the state (e.g., string, number, boolean). Default: string."),
    ...
    call: API => async (args) => {
        let ret = await API.createState(args)
        return {
            content: [
                {
                    type: "text",
