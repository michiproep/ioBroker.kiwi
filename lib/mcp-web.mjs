...
class MCP_Web_Server {
	...
	init() {
		...
		this.adapter.log.info(`MCP Web Server started at ${base}`);
		...
		this.API.init();
		this.adapter.log.info(`API initialized with namespace: ${this.API.namespace}`);
		this.Tools = Tools;
		for (const toolName in this.Tools) {
			const tool = this.Tools[toolName];
			server.tool(tool.name, tool.desc, tool.params, tool.call(this.API));
		}
		this.adapter.log.info(`Tools registered: ${Object.keys(this.Tools).join(", ")}`);
		...
		this.adapter.log.info(`No-cache middleware set up`);
		...
		this.adapter.log.info(`Router attached`);
	}
}
