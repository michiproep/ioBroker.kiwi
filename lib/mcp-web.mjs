...
class MCP_Web_Server {
	...
	init() {
		...
		this.adapter.log.info(`[Kiwi Web] using Api Key: ${this.instanceSettings.apiKey}`);
		this.adapter.log.info(`[Kiwi Web] using Api Key2`);
		this.API = new ioBrokerAdapterApi({
			adapter: this.adapter,
			namespace: this.ref.config.namespace,
			apiKey: this.instanceSettings.apiKey,
