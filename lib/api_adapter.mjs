...
export class ioBrokerAdapterApi {
	...
	async mingoSearch(query, limit = 100, skip = 0) {
		try {
			...
			const res = cursor.skip(skip || 0).limit(limit || 100).all();
	...
	async sendToAsync(instance, command, data) {
        return new Promise((resolve, reject) => {
            try {
                // adapter.sendTo(instance, command, data, cb)
	...
	async getHistory(args) {
        try {
            ...
            // normalize: accept args.options OR top-level start/end/count
            const optionsFromArgs = args?.options || {};
            const options = { ...defaults, ...optionsFromArgs };
            if (typeof args?.start === "number") options.start = args.start;
            if (typeof args?.end === "number") options.end = args.end;
            if (typeof args?.count === "number") options.count = args.count;

            // fallback to last 24h when nothing provided
            // if (!options.start && !options.end) {
            //     options.end = Date.now();
            //     options.start = options.end - 24 * 60 * 60 * 1000;
            // }

            this.logger.debug(`getHistory ${args?.id} options=${JSON.stringify(options)}`);

            // Prefer adapter.getHistoryAsync if available (fast / direct)
            if (this.adapter && typeof this.adapter.getHistoryAsync === "function") {
                const res = await this.adapter.getHistoryAsync(args.id, options);
                this.logger.debug(`getHistory result length=${Array.isArray(res) ? res.length : JSON.stringify(res)}`);
				//Example: {"result":[],"step":null}
				if(res && res.result?.length > 0) 
                	return res;
            }

            // Otherwise try common history adapters via sendTo (n8n / controller setups)
            const candidates = [
                // common adapter instance ids — adjust if you run different instance names
                "influxdb.0",
				"history.0",
                "sql.0",
                "history.1",
            ...
            for (const inst of candidates) {
                try {
                    const resp = await this.sendToAsync(inst, "getHistory", { id: args.id, options });
                    // many history adapters return { result: [...] } or directly an array
                    if (resp && resp.result) return resp.result;
                    if (Array.isArray(resp)) return resp;
                } catch (e) {
                    this.logger.debug(`getHistory sendTo ${inst} failed: ${e?.message || e}`);
                }
            }

            // nothing found -> return empty array (consistent shape)
            this.logger.warn(`getHistory: no history adapter responded for ${args.id}`);
            return [];
        } catch (error) {
