...
export class ioBrokerAdapterApi {
	constructor(options) {
		this.namespace = options.namespace || "mcp-server.0";
		this.adapter = options.adapter;
		this.adapter.log.info("options", options);
	...
	async onObjectChange(id, obj) {
		...
		if (
			...
			this.adapter.log.info(
				`[State Indexed] ${id} has ${this.namespace} custom setting. It is enabled: ${JSON.stringify(obj.common.custom[this.namespace])}`,
			);
		} else {
			try {
	...
	async search(text, filter = {}, limit = 10) {
		try {
			let items = await this.vectorDB.find(text, filter, limit);
	...
	async getAllObjectsInRoom(room) {
		return { status: "not implemented", message: `not imlemented` };
	}
	async getAdapters() {
	...
	async getState(args) {
		try {
			let states = await this.adapter.getForeignStatesAsync(args.id);
	...
	async describe(args) {
		try {
			...
			let obj = await this.adapter.getForeignObjectAsync(args.id);
	...
	async describeBulk(args) {
		let ret = [];
		for (let arg of args) {
			let body = { common: { custom: { "mcp-server.0": { enabled: true, description: arg.desc } } } };
